...
# ── UPDATED: Full terminal coloring with OSC sequences (works perfectly in Termux) ──
apply_colors() {
...
reset_colors() {
    printf '\e]11;?\a'   # Reset background to terminal default
    printf '\e]10;?\a'   # Reset foreground to terminal default
    printf "\e[2J\e[H"
}
...
terminal_bg_color_settings() {
    ...
    case "$b_choice" in
        ...
        2) 
            ...
            reset_colors
            apply_colors
            echo "✅ Background cleared — using terminal default" 
            ;;
        3) 
            terminal_bg_color=$DEFAULT_TERMINAL_BG_COLOR
            save_settings
            ...
            reset_colors
            apply_colors
            echo "✅ Restored to default (→ ${DEFAULT_TERMINAL_BG_COLOR:-terminal default})" 
            ;;
        0) return ;;
        *) echo "❌ Invalid choice" ;;
    esac
...
terminal_text_color_settings() {
    ...
    case "$c_choice" in
        ...
        3) 
            ...
            reset_colors
            apply_colors
            echo "✅ Restored to default (→ ${DEFAULT_TERMINAL_TEXT_COLOR:-terminal default})" 
            ;;
        0) return ;;
        *) echo "❌ Invalid choice" ;;
    esac
